...
async function enviarTexto(telefono: string, texto: string, phoneId: string, token: string) {
...
export const procesarMensaje = action({
  ...
  handler: async (ctx, { telefono, texto, mensajeId }): Promise<void> => {
    ...
    if (step === 0) {
      // Primera respuesta — preguntar nombre
      ...
      // Recibió el nombre — preguntar dolor
      ...
      // Actualizar notas del prospecto con los datos obtenidos
      if (prospect) {
        const notaBot = `[Bot] Nombre: ${nombre} | Dolor: ${dolor}`;
      ...
      if (adminPhone) {
        const nombreNegocio = prospect?.nombre ?? "Desconocido";
        const ciudad = prospect?.ciudad ?? "";
        const alerta =
          `🔔 *Nuevo lead calificado!*\n\n` +
          `🏢 *Negocio:* ${nombreNegocio} (${ciudad})\n` +
          `👤 *Contacto:* ${nombre}\n` +
          `💬 *Su dolor:* "${dolor}"\n` +
          `📱 *Tel:* +${telNorm}`;
